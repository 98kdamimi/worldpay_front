<template>
	<view class="viewBox">
		...
		<view v-if="pageType == 1 || (pageType == 2 && isPassword)">
			...
			<view class="inputBox">
				<input :type="passWordType ? 'password' : 'text'" :placeholder="$t('passwordModify.enterOldPassword')"
					placeholder-style="color: #9191aa;" v-model="requestParams.formerPassword" />
		...
		<view>
			...
			<view class="inputBox">
				<input :type="passWordType1 ? 'password' : 'text'" :placeholder="$t('passwordModify.enterNewPassword')"
					placeholder-style="color: #9191aa;" v-model="requestParams.newPassword" />
				<view @click="passWordType1 = !passWordType1">
					<SvgIcon name="eye3" width="36" height="36" v-if="passWordType1"></SvgIcon>
					<SvgIcon name="eye4" width="36" height="36" v-else></SvgIcon>
				</view>
			</view>
		</view>
		<view>
			...
			<view class="inputBox">
				<input :type="passWordType2 ? 'password' : 'text'"
					:placeholder="$t('passwordModify.reenterNewPassword')" placeholder-style="color: #9191aa;"
					v-model="newPassword2" />
				<view @click="passWordType2 = !passWordType2">
					<SvgIcon name="eye3" width="36" height="36" v-if="passWordType2"></SvgIcon>
					<SvgIcon name="eye4" width="36" height="36" v-else></SvgIcon>
				</view>
			</view>
		</view>
		<view>
			...
			<view class="smallInput">
				<view>
					<!-- 请输入验证码 -->
					<input type="text" :placeholder="$t('passwordModify.emailPassword')"
						placeholder-style="color: #9191aa;" v-model="verifyCode" />
				</view>
				<view class="input-button" :style="{
		...
		<view class="button" @click="getPasswordVerification()">{{ $t('passwordModify.confirmModify') }}</view>
...
<script setup>
	...
	const getPasswordVerification = () => {
		const needCheckOldPassword = (pageType.value == 1) || (pageType.value == 2 && isPassword.value);
		...
		switch (pageType.value) {
			...
			case "2":
				if (!isPassword.value) {
					...
					if (changePayPassword(requestParams.formerPassword, requestParams.newPassword)) {
						uni.showToast({
						...
						uni.showToast({
							title: t('passwordModify.oldPasswordWasEnteredIncorrectly'),
							icon: 'none'
						});
					}
				}
