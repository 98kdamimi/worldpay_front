<template>
	<view class="viewBox" style="padding-bottom: 32rpx;">
		...
		<up-popup :show="showCountryModal" mode="bottom" @close="showCountryModal = false">
			<view class="country-popup">
				<view class="country-popup-header">
					<view class="country-popup-title">{{ $t('select.countryRegios') }}</view>
					<SvgIcon name="off" width="56" height="56" @click="showCountryModal = false"></SvgIcon>
				</view>
				<view class="country-popup-search">
					<input type="text" v-model="countrySearch" :placeholder="$t('select.countryRegiosCode')"
						class="country-search-input" />
				</view>
				<scroll-view scroll-y style="height: 70vh;">
					<view class="country-item" v-for="item in filteredCountryList" :key="item.code"
