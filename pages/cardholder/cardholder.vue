<template>
	<view class="viewBox" style="padding-bottom: 32rpx">
		...
		<view
			class="button"
			@click="goToPage('/pages/addCardholder/addCardholder')"
			v-if="operationType"
		>
			<view>
				<!-- #ifdef APP -->
		...
		>
			<view class="applyModal">
				...
				<view class="applyModal-txt">
					{{ $t('cardholderSelect.confirmDeleteMsg') }}
				</view>
				<view class="applyModal-button">
					<view class="cancel" @click="applyShow = false">
						{{ $t('cardholderSelect.cancel') }}
					</view>
					<view class="apply" @click="getCardholderDelete()">
						{{ $t('cardholderSelect.confirm') }}
					</view>
				</view>
			</view>
		</up-modal>
...
const getCardholderDelete = async () => {
	try {
		...
		getFindByUid();
		uni.showToast({
			title: t('cardholder.deleteSuccess'),
			icon: 'none'
		});
	} catch (error) {
		console.error(error);
